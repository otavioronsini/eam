...
class _RestauranteDetalhePageState extends State<RestauranteDetalhePage> {
  ...
  Future<void> _resgatarRecompensa(Recompensa rec) async {
    ...
    if (sucesso) {
      ...
      // Registra no histórico de pedidos do estabelecimento quem consumiu
      // os pontos (e-mail do cliente) e o que foi comprado.
      await widget.estabelecimentoRepo.registrarPedido(
        Pedido(
          estabelecimentoId: widget.estabelecimento.id!,
          clienteId: widget.clienteId,
          clienteEmail: widget.clienteEmail,
          clienteNome: widget.clienteNome,
          recompensaNome: rec.nome,
          pontosUtilizados: rec.pontosNecessarios,
        ),
      );
